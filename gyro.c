...
/* OVERVIEW OF GYRO READ:
1. Format the passed address (read mode, toggle last bit)
2. Reset gyro_chip_select ODR
3. Wait to finish transmit
4. Set address of gryo to passed address
5. Wait until transmit/receive done

*** VERY IMPORTANT ***
6. Send a dummy byte to the gyro (because a read must follow a write!)

7. Wait until transmit done
8. While receive not empty, read values from the register
9. Pass these values into rBuffer (this value is also returned)
9. Set gyro_chip_select ODR
*/
...
int readGyroRegister(uint8_t addr, uint8_t *rBuffer)
{
	...
	uint8_t RW = 1;
	uint8_t MS = 0;
	uint8_t word = (RW << 7) | (MS << 6); 	// 0x80
	uint8_t value = 0;
	word = word + addr;					 	// Address w/ last bit flipped

	// See pg.307 in RM0351 for BSRR info
	GPIOD->BSRR |= (1U << (2 * 11 + 1));	 // Reset ODR of Gyro_chip_select
	...
	*((volatile uint8_t *)&SPI2->DR) = 0x00; // Sending a dummy byte, doesn't do anything other than completing the read.
	waitForTXE();
	waitForNotBSY();
	while (SPI2->SR & SPI_SR_RXNE)			 // SPI Status Register receive buffer not empty (allowed to read)
	{ // Reads in values from the single specified Gyro register.
		value = *((volatile uint8_t *)&SPI2->DR);
	}
	waitForNotBSY();

	*rBuffer = value;					  // Copy the data read from the gyro into rBuffer 
	
	GPIOD->BSRR |= ((1U) << (2 * 3 + 1)); // Sets ODR of Gyro_Chip_Select

	return value;
}

/* OVERVIEW OF GYRO WRITE:
1. Format the passed address
2. Reset gyro_chip_select ODR
3. Wait to finish transmit
4. Set address of gryo to passed address
5. Wait until transmit/receive done
6. Write the data
7. Wait until transmit/receive done, delay w/ USART
8. Set gyro_chip_select ODR
*/
void writeGyroRegister(uint8_t addr, uint8_t tBuffer)
{
	...
	uint8_t value = 0;
	word = word + addr;						 // Address is unmodified for a write
...
void initializeSPI2(void)
{
	 ...
	 */
	SPI2->CR1 &= ~SPI_CR1_BIDIMODE; // SPI2 in read only mode

	// Output enable in bidirectional mode
	// 0 = Output disabled (recieve-only mode)
	// 1 = Output enabled (transmit-only mode)
	SPI2->CR1 &= ~SPI_CR1_BIDIOE;	// Disable SPI2 bidirectional output (since rx only)

	// Data Frame Format - Sets SPI to send data in 8-bit chunks!
	SPI2->CR2 &= ~SPI_CR2_DS;								// Clears data frame formatting
	SPI2->CR2 = SPI_CR2_DS_0 | SPI_CR2_DS_1 | SPI_CR2_DS_2; // Sets SPI2 to 0111, corresponding to 8-bit format
	...
	// CRC Polynomial- hardware error check transmitted after each byte
	SPI2->CRCPR = 10;

	// Hardware CRC calculation disabled
	SPI2->CR1 &= ~SPI_CR1_CRCEN; // above line isn't needed, we aren't using CRC
	...
	// Allows the software to control how the gyro uses NSS pin (I think?)
	 ...
	 */
	SPI2->CR2 |= SPI_CR2_FRXTH; // Makes RXNE use the threshold value for 8-bit transmission
