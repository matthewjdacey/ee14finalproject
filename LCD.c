...
void LCD_PIN_Init(void){
	...
	// Configure Port C Pins 3, 4, 5, 6, 7, 8 as Alternate Function 11
...
void LCD_Configure(void){
	...
	// Configure DUTY[2:0] of LCD_CR and duty to 1/4 (which is 011)
	LCD->CR &= ~(LCD_CR_DUTY); // clear all duty bits
	LCD->CR |= LCD_CR_DUTY_0; // turn on duty 0th bit
	LCD->CR |= LCD_CR_DUTY_1; // turn on duty 1st bit
	// duty now has 011, which means it is 1/4
	
	// Configure CC[2:0] of LCD_FCR to maximum voltage (this means turning voltage as high as possible, enabling all of the bits)
	LCD->FCR |= LCD_FCR_CC;
	
	// Configure PON[2:0] of LCD_FCR to maximum (same logic as above)
	LCD->FCR |= LCD_FCR_PON;
	
	// Diasage MUX_SEG segment of LCD_CR
	LCD->CR &= ~(LCD_CR_MUX_SEG);
	
	// Select internal voltage as LCD voltage source (means turning off VSEL bit of LCD_CR)
	LCD->CR &= ~(LCD_CR_VSEL);
	
	// enable LCD by setting LCDEN bit of LCD_CR
	LCD->CR |= LCD_CR_LCDEN;
}


// displays a the string onto the screen
void LCD_DisplayString(uint8_t* ptr){
	int pos = 0;
	// loop until we reach end of  the string or end of the screen
	while (*ptr != 0 && pos < 6) {
		LCD_WriteChar(ptr, 0, 0, pos);
		pos++; // move to next position of screen
		ptr++; // move to next character in string
	}
}
...
void LCD_Display_Timer(int ms){
	...
	LCD_WriteChar(&s3, 1, 0, 3);
	LCD_WriteChar(&s4, 0, 0, 4);
	LCD_WriteChar(&s5, 0, 0, 5);
}
...
void LCD_Display_Steps(int steps) {
	...
	LCD_WriteChar(&digit3, 0, 0, 2);
	LCD_WriteChar(&digit4, 0, 0, 1);
	LCD_WriteChar(&digit5, 0, 0, 0);
}
...
void LCD_Display_TOD(int mins) {
	...
	LCD_WriteChar(&min0, 0, 0, 3);
	LCD_WriteChar(&min1, 0, 0, 2);
	LCD_WriteChar(&hr0, 0, 1, 1);
	LCD_WriteChar(&hr1, 0, 0, 0);
}	

void timeUp(void) {
	// Incrementing hours
	if (pos == 0) {
		if (min >= 720)				// If at 12:xx, go to 1:xx
			min -= 660;
		else									// Otherwise increment 1 hour
			min += 60;
	}
	
	// Incrementing minutes
	else if (pos == 1) {	
		if (min % 60 == 59)		// If at xx:59, go to xx:00
			min -= 59;
		else									// Otherwise incrememnt 1 minute
			min++;
	}	
...
void timeDown(void) {
	// Incrementing hours
	if (pos == 0) {
		if (min <= 60)					// If at 1:xx, go to 12:xx
			min += 660;
		else									// Otherwise dcrement 1 hour
			min -= 60;
	}
	
	// Incrementing minutes
	else if (pos == 1) {	
		if (min % 60 == 0)		// If at xx:00, go to xx:59
			min += 59;
		else									// Otherwise decrememnt 1 minute
			min--;
	}	
